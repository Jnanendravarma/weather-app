...
export default async function handler(req, res) {
  ...
  const { q, city, lat, lon, units = 'metric' } = req.query;
  ...
  try {
    ...
    if (q || city) {
      ...
      url = `https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(cityName)}&appid=${apiKey}&units=${units}`;
    } else {
      url = `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${apiKey}&units=${units}`;
    }

    console.log(`Fetching weather data from: ${url.replace(apiKey, 'HIDDEN')}`);
